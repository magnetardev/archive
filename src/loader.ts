...
export default async function load(options: Partial<IModule> = {}): Promise<WasmInterface> {
	const mod = (await initModule({
		locateFile(path, prefix) {
			if (path === "archive.wasm") return new URL("./archive.wasm", import.meta.url).href;
			return prefix + path;
		},
		...options,
	})) as IModule;
	return {
		mod,
		// Read Methods
		read_memory: mod.cwrap("archive_read_memory", "number", ["number", "number", "string"]),
		...
		entry_is_encrypted: mod.cwrap("archive_entry_is_encrypted", "number", ["number"]),
		// Write methods
		write_memory: mod.cwrap("archive_write_memory", "number", ["string", "number"]),
